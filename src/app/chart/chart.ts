...
export class ChartDataContainer {
    ...
    constructor(name, type: CommonEnums.ChartType = null, options = new ExtendedChartOptions(), chartId = null) {
    ...
    public prepareData() {
        ...
        if (this.yPointType === CommonEnums.PointType.Date) {
            ...
            this.dataArray.forEach(dataSeries => {
                dataSeries.dataSeriesPoints.forEach(point => {
                    const time = (point.yPoint as Date).getTime();
                    if (yColumn.findIndex(item => item.getTime() === time) === -1) {
                        yColumn.push(point.xPoint);
                    }
                });
            });
            yColumn.sort((a, b) => {
...
export class DataSeries {
    ...
    constructor(dataSeriesPoints: Array<ChartPoint>,
        type: CommonEnums.ChartType = null, legendName = '', xPointName = '', yPointName = '') {
...
export class ChartPoint {
    ...
    constructor(x, y, object = null, title = '', drawPoint = true) {
        this.xPoint = x;
        this.yPoint = y;
        this.object = object;
        this.title = title;
...
export class ExtendedChartOptions {
    colors = ['#3582E8', '#93bbe7', '#0b5696', '#47BCFF', '#3575E8',
        '#34C6FF', '#3083E8', '#4165FF', '#17B8E8', '#2690FF', '#47BCFF', '#3582E8', '#3A66FF'];
    drawName = false;
    name: string;
    nameColor = '#6c757d';
    nameFontSize = '1rem';
