...
export class ChartBuilder {
    chartId: string;
    private data: ChartDataContainer;
    private options: ExtendedChartOptions;
    private callback: Function;
    private scaleXFunction;
    private svgChart;
    private svgLegend;
    ...
    constructor(callback: Function = null, top = 40, right = 40, bottom = 20, left = 50) {
        this.callback = callback;
        this.chartMargin = new ChartMargin({ top: top, right: right, bottom: bottom, left: left });
    }

    drawChart(data: ChartDataContainer, svgChart, svgLegend) {
    ...
    private sketchLinesGroup(groupType: CommonEnums.GroupType) {
        ...
        if (this.data.yPointType === CommonEnums.PointType.Number) {
            ...
            scaleYFunc.domain(d3.extent(this.data.yColumn, d => d));
        }
    ...
    private sketchLines(data: DataSeries, color, scaleYFunc) {
    ...
    private drawMaxMinText(scaleYFunc) {
        if (this.options.drawMin || this.options.drawMax) {
            ...
            if (this.options.drawMin) {
                ...
                if (this.data.yPointType === CommonEnums.PointType.Date) {
                    ...
                    formattedMinY = Utils.numberFormatter(minY, this.data.yAccuracy);
                }
            ...
            if (this.options.drawMax) {
                ...
                if (this.data.yPointType === CommonEnums.PointType.Date) {
                    ...
                    formattedMaxY = Utils.numberFormatter(maxY, this.data.yAccuracy);
                }
    ...
    private drawLine(g, dataSet, mainColor, scaleYFunc) {
        ...
        let lineFunction = d3.line()
            .x(d => this.scaleXFunction(d.xPoint) + this.scaleXFunction.bandwidth() / 2)
            .y(d => scaleYFunc(d.yPoint));
    ...
    private drawCircle(g, dataSet, mainColor, scaleYFunc) {
        ...
        circle.enter()
            ...
            .attr('cx', 0)
            .on('mouseover', function (cp) { self.onMouseOverElement(d3.select(this), cp); })
            .on('mouseout', function (cp) { self.onMouseLeaveElement(d3.select(this), cp); })
            .transition()
            .duration(this.options.dotAnimationTime)
            .attr('cy', d => scaleYFunc(d.yPoint))
            .attr('cx', (d, i) => this.scaleXFunction(d.xPoint) + this.scaleXFunction.bandwidth() / 2);
            ...
            switch (this.options.nameOfTitleField) {
                case CommonEnums.NamesOfTitleField.yPoint:
                    if (this.data.yPointType === CommonEnums.PointType.Date) {
                        ...
                        g.selectAll('circle').append('svg:title').text((d) => formatTime(d.yPoint));
                    } else if (this.data.yPointType === CommonEnums.PointType.Number) {
                        g.selectAll('circle').append('svg:title').text((d) => Utils.numberFormatter(d.yPoint, this.data.yAccuracy));
                    }
                    break;
                case CommonEnums.NamesOfTitleField.xPoint:
                    if (this.data.xPointType === CommonEnums.PointType.Date) {
                        const formatTime = d3.timeFormat(this.data.dateFormatter);
                        g.selectAll('circle').append('svg:title').text((d) => formatTime(d.xPoint));
                    } else if (this.data.xPointType === CommonEnums.PointType.Number) {
                        g.selectAll('circle').append('svg:title').text((d) => Utils.numberFormatter(d.xPoint, this.data.xAccuracy));
                    }
                    break;
                case CommonEnums.NamesOfTitleField.title:
                    g.selectAll('circle').append('svg:title').text((d) => d.title);
                    break;
                case CommonEnums.NamesOfTitleField.pointXY:
                    g.selectAll('circle').append('svg:title').text((d) => `(${
                        Utils.numberFormatter(d.xPoint, this.data.xAccuracy)},${Utils.numberFormatter(d.yPoint, this.data.yAccuracy)})`);
                    break;
                default:
                    break;
            }
    ...
    private drawCross(g, dataSet, mainColor, scaleYFunc) {
        ...
        leftLine.enter()
            ...
            .transition()
            .duration(this.options.dotAnimationTime)
            .attr('x1', (d, i) => this.scaleXFunction(d.xPoint) + this.scaleXFunction.bandwidth() / 2 - dotCrossLength)
            .attr('y1', (d) => scaleYFunc(d.yPoint) - dotCrossLength)
            .attr('x2', (d, i) => this.scaleXFunction(d.xPoint) + this.scaleXFunction.bandwidth() / 2 + dotCrossLength)
            .attr('y2', (d) => scaleYFunc(d.yPoint) + dotCrossLength);

        const rightLine = g.selectAll('.dot2').data(dataSet);
        rightLine.enter()
            .append('line')
            .attr('class', 'rigth-point-cross')
            .attr('id', d => `rigth${d.pointId}`)
            .attr('stroke', color)
            .attr('stroke-width', this.options.dotBorderWidth)
            .attr('opacity', .7)
            .attr('x1', 0 - dotCrossLength)
        ...
        if (this.options.addValueTitles) {
            switch (this.options.nameOfTitleField) {
                case CommonEnums.NamesOfTitleField.yPoint:
                    if (this.data.yPointType === CommonEnums.PointType.Date) {
                        const formatTime = d3.timeFormat(this.data.dateFormatter);
                        dataSet.forEach(item => g.select(`#left${item.pointId}`).append('svg:title').text(formatTime(item.yPoint)));
                        dataSet.forEach(item => g.select(`#rigth${item.pointId}`).append('svg:title').text(formatTime(item.yPoint)));
                    } else if (this.data.yPointType === CommonEnums.PointType.Number) {
                        dataSet.forEach(item =>
                            g.select(`#left${item.pointId}`).append('svg:title')
                                .text(Utils.numberFormatter(item.yPoint, this.data.yAccuracy)));
                        dataSet.forEach(item =>
                            g.select(`#rigth${item.pointId}`).append('svg:title')
                                .text(Utils.numberFormatter(item.yPoint, this.data.yAccuracy)));
                    }
                    break;
                case CommonEnums.NamesOfTitleField.xPoint:
                    if (this.data.xPointType === CommonEnums.PointType.Date) {
                        const formatTime = d3.timeFormat(this.data.dateFormatter);
                        dataSet.forEach(item => g.select(`#left${item.pointId}`).append('svg:title').text(formatTime(item.xPoint)));
                        dataSet.forEach(item => g.select(`#rigth${item.pointId}`).append('svg:title').text(formatTime(item.xPoint)));
                    } else if (this.data.xPointType === CommonEnums.PointType.Number) {
                        dataSet.forEach(item =>
                            g.select(`#left${item.pointId}`).append('svg:title')
                                .text(Utils.numberFormatter(item.xPoint, this.data.xAccuracy)));
                        dataSet.forEach(item =>
                            g.select(`#rigth${item.pointId}`).append('svg:title')
                                .text(Utils.numberFormatter(item.xPoint, this.data.xAccuracy)));
                    }
                    break;
                case CommonEnums.NamesOfTitleField.title:
                    dataSet.forEach(item => g.select(`#left${item.pointId}`).append('svg:title').text(item.title));
                    dataSet.forEach(item => g.select(`#rigth${item.pointId}`).append('svg:title').text(item.title));
                    break;
                case CommonEnums.NamesOfTitleField.pointXY:
                    dataSet.forEach(item => g.select(`#left${item.pointId}`).append('svg:title')
                        .text(`(${Utils.numberFormatter(item.xPoint, this.data.xAccuracy)},${
                            Utils.numberFormatter(item.yPoint, this.data.yAccuracy)})`));
                    dataSet.forEach(item => g.select(`#rigth${item.pointId}`).append('svg:title')
                        .text(`(${Utils.numberFormatter(item.xPoint, this.data.xAccuracy)},${
                            Utils.numberFormatter(item.yPoint, this.data.yAccuracy)})`));
                    break;
                default:
    ...
    private drawSquare(g, dataSet, mainColor, scaleYFunc) {
        ...
        square.enter()
            ...
            .duration(this.options.dotAnimationTime)
            .attr('x', (d, i) =>
                this.scaleXFunction(d.xPoint) + this.scaleXFunction.bandwidth() / 2 - this.options.dotBorderSquareSideLength / 2)
            .attr('y', (d) => scaleYFunc(d.yPoint) - this.options.dotBorderSquareSideLength / 2);
        ...
        if (this.options.addValueTitles) {
            switch (this.options.nameOfTitleField) {
                case CommonEnums.NamesOfTitleField.yPoint:
                    if (this.data.yPointType === CommonEnums.PointType.Date) {
                        const formatTime = d3.timeFormat(this.data.dateFormatter);
                        g.selectAll('rect').append('svg:title').text((d) => formatTime(d.yPoint));
                    } else if (this.data.yPointType === CommonEnums.PointType.Number) {
                        g.selectAll('rect').append('svg:title').text((d) => Utils.numberFormatter(d.yPoint, this.data.yAccuracy));
                    }
                    break;
                case CommonEnums.NamesOfTitleField.xPoint:
                    if (this.data.xPointType === CommonEnums.PointType.Date) {
                        const formatTime = d3.timeFormat(this.data.dateFormatter);
                        g.selectAll('rect').append('svg:title').text((d) => formatTime(d.xPoint));
                    } else if (this.data.xPointType === CommonEnums.PointType.Number) {
                        g.selectAll('rect').append('svg:title').text((d) => Utils.numberFormatter(d.xPoint, this.data.xAccuracy));
                    }
                    break;
                case CommonEnums.NamesOfTitleField.title:
                    g.selectAll('rect').append('svg:title').text((d) => d.title);
                    break;
                case CommonEnums.NamesOfTitleField.pointXY:
                    g.selectAll('rect').append('svg:title').text((d) => `(${
                        Utils.numberFormatter(d.xPoint, this.data.xAccuracy)},${Utils.numberFormatter(d.yPoint, this.data.yAccuracy)})`);
                    break;
                default:
                    break;
    ...
    private drawBar(g, dataSet, mainColor, scaleYFunc, maxY) {
        ...
        bars.enter()
            ...
            .attr('opacity', .7)
            .attr('x', (d, i) => this.scaleXFunction(d.xPoint))
            .attr('width', this.scaleXFunction.bandwidth())
            .attr('y', scaleYFunc(0))
            .attr('height', 0)
            .on('mouseover', function (cp) { self.onMouseOverElement(d3.select(this), cp); })
            .on('mouseout', function (cp) { self.onMouseLeaveElement(d3.select(this), cp); })
            .on('click', (d) => self.onMouseClick(d))
            .transition()
            .duration(this.options.barAnimationTime)
            .attr('y', d => scaleYFunc(d.yPoint))
        ...
        if (this.options.additionalTextForPoint) {
            bars.enter()
            .append('text')
            .attr('x', (d, i) => this.scaleXFunction(d.xPoint) + 15)
    ...
    private drawText(g, scaleYFunc, yText, xPos, yPos) {
    ...
    private drawXAxis() {
        ...
        if (this.options.drawXTicks) {
            if (this.data.xPointType === CommonEnums.PointType.Date) {
                g.append('g').attr('class', 'axis axis-x')
                    ...
                    .call(d3.axisBottom(this.scaleXFunction)
                ...
                g.append('g').attr('class', 'axis axis-x')
                    ...
                    .call(d3.axisBottom(this.scaleXFunction)
                        .ticks(this.options.xTicksNumber)
                        .tickFormat(d => Utils.numberFormatter(d, this.data.xAccuracy)));
            }
        } else {
            g.append('g').attr('class', 'axis axis-x')
                .attr('transform', 'translate(0,' + this.chartHeight + ')')
        ...
        if (this.options.rotateXticks) {
            g.selectAll('.tick text')
                .attr('transform', 'rotate(-45) translate(-20, -5)');
        }
    ...
    private drawYAxis(scaleYFunc, groupType: CommonEnums.GroupType) {
        const g = this.svgChart.select('g#chart-main');
        ...
        if (this.options.drawYTicks) {
            if (this.data.yPointType === CommonEnums.PointType.Number) {
                g.append('g')
                    ...
                    .call(axisFunc
                        .ticks(this.options.yTicksNumber)
                        .tickFormat(d => Utils.numberFormatter(d, this.data.yAccuracy)));
            } else if (this.data.yPointType === CommonEnums.PointType.Date) {
                g.append('g')
                    .attr('class', axisClass)
                    .attr('transform', 'translate( ' + translate + ', 0 )')
    ...
    private drawYGrid(scaleYFunc) {
        if (this.options.drawGrid) {
            this.yGridDrawn = true;
            this.svgChart.select('g#chart-main')
                .append('g').attr('class', 'grid')
                .call(d3.axisLeft(scaleYFunc)
                    .ticks(5).tickSize(-this.chartWidth)
                    .tickFormat(''));
    ...
    private styleGrid() {
        d3.selectAll('.grid path')
            .style('stroke-width', this.options.gridWidth);
    ...
    private wrap(width, g) {
        const texts = g.selectAll('text');
        texts._groups[0].forEach(item => {
    ...
    private onMouseClick(element) {
        if (this.callback) {
            this.callback(element, 'click');
        }
    }

    private onMouseOverElement(d3Element, element) {
    ...
    private onMouseLeaveElement(d3Element, element) {
    ...
    private highlightElement(d3Element) {
        d3Element.attr('opacity', 1);
    }

    private unHighlightElement(d3Element) {
        d3Element.attr('opacity', .7);
    }

    public onMouseOver(id) {
        const g = this.svgChart.select('g#chart-main');
        const d3Element = g.select(`#${id}`);
        this.highlightElement(d3Element);
    }

    public onMouseOut(id) {
        const g = this.svgChart.select('g#chart-main');
        const d3Element = g.select(`#${id}`);
        this.unHighlightElement(d3Element);
    }
